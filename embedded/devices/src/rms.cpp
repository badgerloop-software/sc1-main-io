...
#define TORQUE_SCALE_LWR(x) \
  (((x)&0xFF) * 10) /* Converts Nm to the value the RMS wants */
...
int RMS::parser(uint32_t id, uint8_t* rmsData, uint32_t filter) {
  ...
  switch (id) {
    case (0xa0):
      ...
      //        setRmsGateDriverBoardTemp((rmsData[6] | (rmsData[7] << 8)) /
      //        10); //Deg C
      /*
              printf("IGBT: %d\r\n", getRmsIgbtTemp());
      ...
      */
      break;
    case (0xa1):
      //        setRmsControlBoardTemp((rmsData[0] | (rmsData[1] << 8)) / 10);
      //        // Deg C
      /*
    ...
    case (0xa4):
      break;
    case (0xa5):
      //        setRmsMotorSpeed((rmsData[2] | (rmsData[3] << 8))); //val == 0 ?
      //        0 : 65536 - abs(val) ;//< -10000 || val > 10000 ?
      //        getRmsMotorSpeed(): val; // RPM setRmsElectricalFreq((rmsData[4]
      //        | (rmsData[5] << 8)) / 10); //electrical frequency Hz
      /*
    ...
    case (0xaa):
      //         setRmsCanCode1((rmsData[3] << 24) | (rmsData[2] << 16) |
      //         (rmsData[1] << 8) | rmsData[0]); setRmsCanCode2((rmsData[7] <<
      //         24) | (rmsData[6] << 16) | (rmsData[5] << 8) | rmsData[4]);
      /*
      ...
      */
      break;
    case (0xab):
      //      setRmsFaultCode1((rmsData[3] << 24) | (rmsData[2] << 16) |
      //      (rmsData[1] << 8) | rmsData[0]);
      //        setRmsFaultCode2((rmsData[7] << 24) | (rmsData[6] << 16) |
      //        (rmsData[5] << 8) | rmsData[4]);
      /*
      ...
      */
      break;
    case (0xac):
      //        setRmsCommandedTorque((rmsData[0] | (rmsData[1] << 8))); // >
      //        200 ? getRmsCommandedTorque() : val;
